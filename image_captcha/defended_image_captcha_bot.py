...
"""
uses pygame to create a GUI for the defended image captcha system
will display 9 random adversarial images of cats and dog from the advCat/advDog folders
the image are passed to yolov5m and it will mark cat images in green, dog images in red and if non were found, in black.
"""


#defining colors as tuples. If a dog was found, mark its image in red. 
...
#listing all the images if the advDog and advCat directories
DogDirectory = 'advDog/' 
CatDirectory = 'advCat/'
    ...
    while not done:
        ...
        for box in Boxes:
            ...
            if box.color == GRAY:
                ...
                text = model(box.path).pandas().xyxy[0].value_counts('name')
                if "cat" in text:
                    box.update(GREEN)
                elif "dog" in text:
                    box.update(RED)
